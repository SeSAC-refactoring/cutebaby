...
// 사용자 정보 조회 처리
export const newgrow = async (req, res) => {
 
    try {
        const { babyid, height , weight , head , inputData } = req.body; // 클라이언트에서 받은 이메일
        console.log(req.body);
        if (!height || !weight || !head || !babyid) {
            return res.status(400).json({ success: false, message: "모든 성장 정보가 필요합니다." });
        }
        const newGrowthRecord = await newgrowModel({
            babyid,
            height,
            weight,
            head,
