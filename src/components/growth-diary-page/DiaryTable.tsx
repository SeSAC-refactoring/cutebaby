...
export const DiaryTable: React.FC<DiaryTableProps> = ({ growData, onEdit, onDelete }) => {
    return (
        <div className={styles.list_wrap}>
            ...
            <div>
                {growData.length > 0 ? (
                    <ul>
                        {growData.map((info, i) => (
                            <li key={i} className={styles.row_el}>
                                ...
                                <button className={`${styles.table_btn} ${styles.edit_btn}`}
                                    onClick={() => onEdit(info.id)}>
                                    수정
                                </button>
                                <button className={`${styles.table_btn} ${styles.delete_btn}`}
                    ...
                    <ul><li className={styles.row_el}>성장 기록이 없습니다.</li></ul>
                )}
            </div>
        </div>
    );
