...
export const UserupdateModal: React.FC<UserupdateProps> = ({ modalState }) => {
    ...
    const handleUpdate = async () => {
        ...
        if (!emailbtn) {
            setMessages((prev) => ({ ...prev, email: "이메일 중복 체크를 해주세요!" }));
            return;
        }
        ...
        try {
            const response = await axios.post("http://localhost:5001/api/updateUser", {
                email: formData.email,
            ...
            <div className={styles.modal_background}>
                <div className={styles.modal_container}>
                    ...
                    <div className={styles.modal_button_container}>
                        <button className={`${styles.modal_btn} ${styles.modal_cancel_button}`} onClick={modalState}>
                            취소
                        </button>
                        <button className={`${styles.modal_btn} ${styles.modal_done_button}`} onClick={handleUpdate}>
                            완료
                        </button>
                    </div>
                </div>
