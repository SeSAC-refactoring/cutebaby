...
app.post('/api/getPosts', (req, res) => {
    ...
    const getUserNumberQuery = 'SELECT usernumber FROM userinfo WHERE userid = ?'; // mysql로 보내는 쿼리문
  
    db.query(getUserNumberQuery, [userid], (err, result) => { //db에 보내는 쿼리는 (요청 쿼리문 , [같이 보내는 값 = 쿼리문 안에 ?에 들어가는값 ] , 콜백받는 값)
      if (err) {
      ...
      const getUserPostsQuery = 'SELECT * FROM user_post WHERE usernumber = ?';
